...
txn.module @MultiCycle {
  ...
  txn.action_method @readMemory(%addr: i32) attributes {timing = "static(2)"} {
    txn.yield
  }
  
  // Static 5-cycle operation
  txn.action_method @process() attributes {timing = "static(5)"} {
    txn.yield
  }
  
  // Dynamic timing operation  
  txn.action_method @conditionalOp(%cond: i1) attributes {timing = "dynamic"} {
    txn.if %cond {
      txn.yield
    } else {
      txn.yield
...
// CHECK:     // TODO: Implement action logic
// CHECK:   }
